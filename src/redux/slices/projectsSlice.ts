  ...
  reducers: {
    ...
    updateProject: (
      ...
      action: PayloadAction<
        Project | { id: number; name: string; description: string; status: ProjectStatus }
      >
    ) => {
    ...
    setCreatedTicket: (state, action: PayloadAction<Ticket>) => {
      state.createdProjects
        .find((p) => p.id === action.payload.project.id)
    ...
    deleteTicket: (
      state,
      action: PayloadAction<Ticket | { id: number; project: { id: number } }>
    ) => {
      // Delete the ticket from the state
      let project = state.createdProjects.find((p) => p.id === action.payload.project.id);
      if (!project) project = state.otherProjects.find((p) => p.id === action.payload.project.id);
      if (project) project.tickets = project.tickets.filter((t) => t.id !== action.payload.id);
    },
    updateContributor: (state, action: PayloadAction<Project>) => {
      ...
      if (project) project.contributors = action.payload.contributors;
    },
        ...
        if (ticket) {
          ...
          if (action.payload.assigneeId) {
            ...
            if (action.payload.assigneeId === ticket.author.id) {
              ...
              ticket.assignee = project.contributors.find(
                (c) => c.id === action.payload.assigneeId
              );
            }
          } else {
...
export const createAndLoadProject = (
  data: ProjectCreateInput,
  setLoading: React.Dispatch<React.SetStateAction<boolean>>,
  closeDrawer: () => void,
  reset: UseFormReset<ProjectCreateInput>
): AppThunk => {
...
export const createAndLoadTicket = (
  data: CreateTicketApiData,
  setLoading: React.Dispatch<React.SetStateAction<boolean>>,
  closeDrawer: () => void,
  reset: UseFormReset<TicketCreateInput>
): AppThunk => {
  return async (dispatch, getState) => {
    try {
      const user = getState().auth.user;
...
export const updateAndLoadProject = (
  data: ProjectUpdate,
  onSuccess?: () => void,
  onError?: () => void
): AppThunk => {
  return async (dispatch, getState) => {
    try {
      ...
      dispatch(setReqStatus({ name: 'updateAndLoadProject', status: 'failed' }));
      dispatch(updateAndShowNotification({ severity: 'error', message: 'Internal Server Error' }));
      if (onError) onError();
    }
  };
...
export const updateAndLoadTicket = (
  data: UpdateTicketApiData,
  onSuccess?: () => void,
  onError?: () => void
): AppThunk => {
  return async (dispatch, getState) => {
...
export const deleteTicketThunk = (
  ticket: { id: number; project: { id: number } },
  onSuccess?: () => void
): AppThunk => {
  return async (dispatch, getState) => {
    try {
...
export const addContributorThunk = (
  data: AddContributor,
  onSuccess?: () => void,
  onError?: () => void
): AppThunk => {
...
export const {
  demoUpdateTicket,
  demoSetCreatedTicket,
  demoAddContributor,
  demoRemoveContributor,
  setProjects,
  setReqStatus,
  setError,
  resetProjects,
