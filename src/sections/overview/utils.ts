        ...
        p.tickets.forEach(t => {
            if (t.asigneeId === userId) {
                if (t.type === 'TASK')
                    ...
                    data.ticketTypeCount.BUG += 1;

                if (t.status !== 'CLOSED') {
                    data.activeTickets += 1;

                    if (t.priority === 'URGENT')
                        data.ticketPriorityCount.URGENT += 1;
                    else if (t.priority === 'HIGH')
                        data.ticketPriorityCount.HIGH += 1;
                    else if (t.priority === 'NORMAL')
                        data.ticketPriorityCount.NORMAL += 1;
                    else
                        data.ticketPriorityCount.LOW += 1;
